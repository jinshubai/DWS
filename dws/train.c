...
void exit_with_help()
{
	...
	"-C : find parameter C (only for -s 0 and 1)\n"
	"-t warm start: set warm start algorithm to find a best parameter C\n"
	"    0 -- warm start is not set\n"
	"    1 -- warm start is set\n"
...
void parse_command_line(int argc, char **argv, char *input_file_name, char *model_file_name)
{
	...
	if(flag_find_C)
	{
		...
		{
			if(mpi_get_rank()==0) 
				fprintf(stderr, "Solver not specified. Using -s 1\n");
			param.solver_type = L2R_L2LOSS_SVC;
		}
		else if(param.solver_type != L2R_LR && param.solver_type != L2R_L2LOSS_SVC)
		{
			if(mpi_get_rank()==0) 
			fprintf(stderr, "Warm-start parameter search only available for -s 0 and -s 1\n");
			exit_with_help();
		}
	}
